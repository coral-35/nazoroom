...
`db reset` 後のイベントは未開始です。管理ページを直接開き、`探索を開始` を押すと、その時刻から指定分数の制限時間が始まります。

```txt
http://localhost:3001/admin/events/00000000-0000-0000-0000-000000000001
```

管理ページでは以下を操作できます。

- `探索を開始`: `starts_at` を現在時刻、`ends_at` を現在時刻 + 制限時間に設定
